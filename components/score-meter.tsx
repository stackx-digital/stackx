...
/** Score band → color. Relative percentiles, so bands read as strong/mid/weak. */
  ...
  return (
    <div
      ...
      aria-label={STAGES.map(
        (s) => `${s.label} ${values[s.key] === null ? "N/A" : values[s.key]}`,
      ).join(", ")}
    >
      {STAGES.map((stage) => {
        ...
        return (
          <div key={stage.key} className="flex-1">
            <div
              className={cn(
                "relative w-full overflow-hidden rounded-full bg-ink/60",
                trackHeight,
              )}
            >
              <div
                ...
                style={{ width: v === null ? "100%" : `${v}%`, opacity: v === null ? 0.2 : 1 }}
              />
            </div>
            {size === "default" && (
              <div className="mt-1 flex items-center justify-between text-[10px] text-muted-foreground">
                <span className="uppercase tracking-wide">{stage.short}</span>
                <span className="tabular">{v === null ? "N/A" : v}</span>
              </div>
            )}
          </div>
        );
